...
class BulletFormatter:
    ...
    def apply_bullet_formatting(self, paragraph, formatting: Dict[str, Any], text: str, fallback_formatting: Dict[str, Any] = None) -> None:
        ...
        try:
            ...
            # Get the detected bullet marker and separator
            bullet_marker = formatting.get('bullet_marker', '•') if formatting else '•'
            bullet_separator = formatting.get('bullet_separator', ' ') if formatting else ' '
            ...
            # Try to get marker from formatting, then fallback, then default
            if formatting and formatting.get('bullet_marker'):
                marker = formatting['bullet_marker']
                separator = formatting.get('bullet_separator', ' ')
            elif fallback_formatting and fallback_formatting.get('bullet_marker'):
                marker = fallback_formatting['bullet_marker']
                separator = fallback_formatting.get('bullet_separator', ' ')
            else:
                marker = '•'
                separator = ' '
    ...
    def _find_bullet_formatting(self, doc: Document, project_info: Dict) -> Dict[str, Any]:
        ...
        for i in range(search_start, search_end):
            ...
            if para_formatting:
                ...
                if detected_marker:
                    found_markers.append((detected_marker, para_formatting))
        
        # Use the first found marker (prioritize local consistency)
        if found_markers:
            detected_marker, para_formatting = found_markers[0]
            logger.info(f"Found local bullet marker in project section: '{detected_marker}'")
    ...
    def _detect_document_bullet_marker(self, doc: Document) -> str:
        ...
        for para in doc.paragraphs:
            ...
            if text:
                ...
                for marker in ['•', '●', '◦', '▪', '▫', '‣', '*', '-']:
                    # Check for both space and tab separators
                    if text.startswith(marker + ' ') or text.startswith(marker + '\t'):
                        marker_counts[marker] = marker_counts.get(marker, 0) + 1
                        break
                    # Also check for markers without separator (tight formatting)
        ...
        # Return the most common marker (prioritize document consistency)
        if marker_counts:
            # Sort by count (descending) - use whatever is most common in the document
            sorted_markers = sorted(marker_counts.items(), 
                                  key=lambda x: x[1], 
                                  reverse=True)
            most_common_marker = sorted_markers[0][0]
            logger.info(f"Detected most common bullet marker: '{most_common_marker}' (count: {sorted_markers[0][1]})")
